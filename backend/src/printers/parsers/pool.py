...
# КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: отдаем управление, как только готов DOM-скелет страницы.
# Это предотвратит зависание рендерера Chrome на скриптах Kyocera.
...
class DriverPool:
    def __init__(self, max_drivers: int = 3, max_usage: int = 50):
        ...
        self.max_usage = max_usage  # Максимальное число запросов перед перезапуском браузера
        self._available: List[webdriver.Chrome] = []
        self._in_use: List[webdriver.Chrome] = []
        self._driver_usage: Dict[int, int] = {}  # id(driver) -> count
    ...
    def acquire(self, timeout: float = 30.0) -> webdriver.Chrome:
        ...
        with self._condition:
            while not self._available:
                ...
                if remaining <= 0:
                    raise TimeoutError("Превышено время ожидания свободного драйвера в пуле")
    ...
    def release(self, driver: webdriver.Chrome) -> None:
        with self._condition:
            if driver in self._in_use:
                ...
                # Борьба с утечками памяти: проверяем счетчик использования браузера
                d_id = id(driver)
                self._driver_usage[d_id] += 1

                if self._driver_usage[d_id] >= self.max_usage:
                    logger.info(f"Перезапуск процесса Chrome {d_id} из-за лимита использований ({self.max_usage})")
                    try:
                        driver.quit()
                    except Exception:
                        pass
